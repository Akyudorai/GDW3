...
public class Controller : MonoBehaviour
{
    ...
    protected float ClampAngle(float angle, float from, float to)
    {
    ...
    {
        var layerMasks = LayerMask.GetMask("Ground", "Interactable");
        if (Physics.Raycast(transform.position + Vector3.up * 0.1f, transform.TransformDirection(-Vector3.up), out RaycastHit hit, 0.5f, layerMasks))
        {
