...
public class SplineUtils : MonoBehaviour
{
    public static SplinePath GenerateWallRunPath(Vector3 position, Vector3 direction, float speed, bool isForward) 
    {
        ...
        p2.transform.position = position + direction * (speed * 0.5f) * inverter + Vector3.up;
        p3.transform.position = position + direction * (speed * 1.0f) * inverter + Vector3.up * 1.5f;
        p4.transform.position = position + direction * (speed *1.5f) * inverter + Vector3.up;
        p5.transform.position = position + direction * (speed * 2) * inverter; 
